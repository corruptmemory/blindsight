...
lazy val docs = (project in file("docs")).enablePlugins(ParadoxPlugin, ParadoxSitePlugin).settings(
...
lazy val semantic = (project in file("semantic")).settings(
  name := "blindsight-semantic",
  libraryDependencies += "org.scala-lang.modules" %% "scala-java8-compat" % "0.9.1" % Test,
  libraryDependencies += logbackClassic % Test,
  libraryDependencies += logstashLogbackEncoder % Test,
  libraryDependencies += scalaTest % Test
).dependsOn(slf4j, api).dependsOn(fixtures % "compile->compile;test->test")

lazy val fluent = (project in file("fluent")).settings(
...
lazy val logstash = (project in file("logstash")).settings(
